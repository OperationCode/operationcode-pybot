...
async def test_detect_credentials(bot: SirBot, aiohttp_client, headers, status):
    ...
    res = await client.get("/pybot/api/v1/slack/verify?email=test@test.test", headers=headers)
...
async def test_verify_returns_correct_success_params(bot: SirBot, aiohttp_client):
    ...
    res = await client.get("/pybot/api/v1/slack/verify?email=test@test.test", headers=AUTH_HEADER)
