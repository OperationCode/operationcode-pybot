...
async def handle_daily_programmer_post(parsed_body: MessageReceivedChannelEvent, context: AsyncBoltContext) -> None:
    await context.ack()
    ...
    if post_id and post_count:
        daily_programmer_table.update(post_id, {"Post Count": post_count, "Last Posted On": datetime.now(timezone.utc)})
        return None
                ...
                daily_programmer_table.create_record(
                    {
                        ...
                        "Initially Posted On": datetime.fromtimestamp(float(body.ts), timezone.utc),
                        "Last Posted On": datetime.fromtimestamp(float(body.ts), timezone.utc),
                        "Posted Count": 1,
